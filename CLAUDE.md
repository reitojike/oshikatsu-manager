...
300行未満が51%)を、mainにマージ済みのPR 26本(#9〜#63)で検証した結果、
差分の中央値135行、300行以下が65%で、**このリポジトリは外部の実測より小さい。**
したがって300行は「現状を追認する上限」であって、これに合わせるために進め方を変える必要はない。
超えた5本(#16 3528行 / #32 1063行 / #41 698行 / #18 679行 / #29 617行)のうち、#16は3302行が
`yarn.lock`(除けば226行)、#32はほぼ全部が権限マトリクスをテストに写した行だった。
**行数だけを見ると、実際には判断が少なかったPRが引っかかる。**上記の除外規定はここから来ている。
...
機械的に弾けば、上記のとおり生成物やテスト表で誤検知し、通すことが目的の中身のない分割を生む。
