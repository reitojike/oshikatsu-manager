...
- **1 Issueに含める「何を選ぶか」は3個まで。5個を超えるなら、着手前に分ける。**
...
**なぜこの数字か。**元の根拠は外部の実測(1 Issueの決めごとが3個なら成功率73%、20個なら12%。
PR差分の中央値286行、300行未満が51%)だが、**このリポジトリの実績で裏を取ったうえで採っている。**
mainにマージ済みのPR 26本(#9〜#63)は差分の中央値135行、300行以下が65%で、外部の実測より小さい。
