...
// 候補1件の評価(gh呼び出し・git statusを含む)がどこかで例外を投げると、既定モード
// (報告のみ)の目的である「その時点までに判定できた白/黒を報告する」ことすらできなくなる。
// 候補単位で例外を吸収し、その候補だけ黒(evaluation-failed)として報告し、残りの候補の
// 判定を続ける。頻度が高いのはgh呼び出し(API・レート制限あり)だが、catchの範囲はそれに
// 限らない(例: 判定の合間にworktreeが削除されてgit statusが失敗する場合も同様に扱う)。
