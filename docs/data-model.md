...
- 削除ガードの `is_admin` 例外を**MVPスコープ外**に変更。カラムは用意するが、
  それを参照する権限判定はRLSにもアプリ層にも実装しない(拡大期へ)
...
> 管理者画面と強制削除は拡大期(`docs/roadmap.md`「拡大期バックログ」)。
...
> 権限判定を分岐させない」こととは別の話で、拡大期で管理者判定を実装する前に
...
3. **削除できないイベントの扱い**
   ...
   MVPでは運用でカバーし、拡大期で管理者画面(`is_admin` ユーザー向け)を用意して
   強制削除できるようにする(`docs/roadmap.md`「拡大期バックログ」)。
