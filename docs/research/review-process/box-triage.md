...
- **必須ゲートの「0件」が、前の回に自分が出した未処置のP1を再掲しないことがある
  (計画セッション実測・2026-08-17、本PR自身で発生。判定: 箱1)。**`00:31:19Z`・`00:31:28Z`の
  claude-reviewはP1を2件出した(本節末尾の項目——`徹底的なコードレビュー`トグルを箱2「C側に
  言及なし」としていたが実際は`docs/pr-review-flow-details.md:507`に記録がある、および
  `reviews.enabled`を箱3に置いていたが実際は`.coderabbit.yaml`に存在しないキー)。**その後の
  2回のclaude-review(`f0a5c94`・`3d0fd56`)はいずれも「P0/P1 0件」と報告したが、この2件は
  本文に未処置のまま残っていた。**「0件」だけを見てゲート充足と判断すると、前の回のP1が
  黙って落ちる——`docs/review-process-design.md`§8.5「不在を成功の根拠にしない」が想定する形の、
  別の現れ方である(「不在」がここでは「前回の指摘が今回の出力に無い」という形で出た)。
  **実際にこの構造どおりの見落としが起きた**(本Issueの完了サマリコメント投稿の39秒後に
  計画セッションが2件の未処置を指摘)。なぜ再掲されなかったかの機構は特定できていない
  (差分には含まれているため「見えなかった」では説明できない)。**処置は#258。本Issueは
  記録まで。**
...
| 軸2: 個人設定に「徹底的なコードレビュー」トグルが存在する(A2、Codex Cloud設定画面) | `docs/pr-review-flow-details.md:507`に現在値`OFF`(2026-08-13確認、設定画面で目視、根拠#186)が記録されている | 一致(**claude-review指摘・2026-08-17T00:31:19Zで箱2から訂正**。当初「C側に言及なし、状態不明」として3.2節の箱2に置いていたが、`docs/pr-review-flow-details.md`の「外部設定の意図する値」表に実測記録があり、台帳の機構記述と値として突き合わせ可能なため箱1に移した) |
...
**Draft期の実測(本PR #269自身、2026-08-16〜17。計画セッション集計)。**予想どおり、
Codex・CopilotはDraft期間中は0回だった(Ready化していないため)。

| 系統 | Draft期の起動回数 | 指摘件数 |
| --- | --- | --- |
| CodeRabbit | 6回(push毎の自動発火) | インライン13件+diff範囲外1件=14件 |
| claude-review | 5回以上(`review:full`の明示依頼) | P1 4件 |
| Copilot | 0回 | — |
| Codex Cloud | 0回 | — |

**これは「出した件数」であって「妥当だった件数」ではない。**CodeRabbitの14件中、
少なくとも1件(「2026-08-17は未来の日付」というMajor指摘)は誤検知だった
(6.4節の日付基準の議論を参照。UTC/JST差によるもので、指摘どおりに日付を書き換えていたら
正しい記録を壊していた)。**「Codex・Copilotは1PRに1回、CodeRabbitはDraft中も含めて都度」
という予想は、Draft期だけを見ても(Codex・Copilotの0回を含めて)整合している**
(1回になるのはReady化後)。
...
一次情報との食い違いとして同じPRで修正した(軸10の行と値域表。CodeRabbit指摘・2026-08-17で
本節の記述を台帳の水準に合わせて修正——反証と正の主張の強さを非対称に保つ)。**`draft`が原因という
帰属は反例1件で反証済み**(`cli/cli` #14104はdraft:true・非forkで非空)。**一方forkについては、
標本2件(`cli/cli` #14104・`vercel/next.js` #97420)でいずれも空だったという観測にとどまる
(`vercel/next.js` #97420はdraft:false・forkで空)。「forkが決定変数」と断定しない。**
2026-08-16観測。**これは3箱の仕分け結果ではない(#257本文の指定どおり、混ぜていない)。**

**あわせて記録する: この指摘はCodeRabbitのインラインコメントではなく、レビュー本文の
「Outside diff range comments」の折りたたみの中にあった(計画セッション確認・2026-08-17)。**
インラインコメント面(面4)だけを見ていると落ちる実例であり、`.claude/skills/pr-review-flow/SKILL.md`
「指摘の扱いとマージ」が求める「面3を折りたたみを含めて全文読む」運用が実際に効いた1件として、
CodeRabbitの箱1(2.3節)の材料に加える。
...
**さらに、claude-reviewが`00:31`に出した未処置のP1 2件を反映した(計画セッション指摘・
2026-08-17。完了サマリ投稿の39秒後に指摘が届いた)。**Codexの「徹底的なコードレビュー」トグルを
箱2「C側に言及なし」としていたが実際は`docs/pr-review-flow-details.md:507`に記録があり、
箱1(2.2節)へ再分類。CodeRabbitの`reviews.enabled`を箱3に置いていたが実際は`.coderabbit.yaml`に
存在しないキーであり削除(2.1節90行で既に「実在しない」と書いていたのと矛盾していた)。
**あわせて、claude-reviewの「0件」報告が前の回の未処置P1を再掲しないことがある、という
実測を新しい箱1の項目として2.1節に追加した**(このIssue自身のPR #269で実際に発生した見落とし)。
