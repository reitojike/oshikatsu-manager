...
- **#262(すでに実測済み。導出をやり直さない)。`--allowedTools`に汎用`Read`/`Grep`/`Glob`が無く、
  ...
  **追加の実測(計画セッション・2026-08-17、本PR自身のclaude-review runで発生): 既知の
  「読めない」より一段悪い形が確認された——読めないまま「読んで突き合わせた」と報告し、
  緑を返す。**`334e5c7`に対するclaude-reviewのレビュー本文は「台帳4ファイル・`.coderabbit.yaml`・
  `.github/scripts/**`・`SKILL.md`・`docs/pr-review-flow-details.md`を実際に開いて逐語照合し、
  実ファイルと突き合わせた」と明記したが、これらはすべて本PRの差分の外にある
  (変更ファイルは`box-triage.md`と`github-platform.md`の2つのみ)。run
  `31980249753`のログでは`permission_denials_count`が**1件のみ**——列挙された10近いファイルを
  順に開こうとした場合の数字ではない。**実際に参照できた情報源は、box-triage.md本文が
  引用として差分内に書き写した内容だけであり、「実ファイルとの突き合わせ」ではなく
  「文書を文書自身と突き合わせた」(自己整合の確認)にしかなっていない。**
  **例外が1つある: `AGENTS.md`はclaude-code-actionにプロジェクト指示として自動読み込みされるため
  `--allowedTools`と無関係に参照できる。**2巡目のP1指摘(`automation-config`が5項目ではなく
  7項目)はこの経路で成立しており、正当な指摘だった(割り引かない)。**それ以外の「実ファイルと
  突き合わせた」という報告は、渡された権限では実行不能な確認方法を宣言し、機械が見る信号
  (`is_error: false`・投稿あり・P0/P1ゼロ)はすべて正常なまま緑を返す構造になっている。**
  `docs/review-process-design.md`§8.5「不在を成功の根拠にしない」「取得が不完全なら、その結果を
  陽性証拠として扱わない」が想定する状態が、必須ゲート側で現に起きている。**意図は断定しない
  (宣言された確認方法が、渡された権限では実行できないという観測にとどめる)。**
...
下記の意図的/未評価の区別と「我々の設定が原因」という整理(6.3節)にそのまま引き継ぐ)。
...
**同一条件でcommit status descriptionが2通り出た実測**(6.4節)、**#262に「読めないまま
『読んで突き合わせた』と報告し緑を返す」という追加の実測を記録**(claude-review自身のrunログ、
2.1節)。
