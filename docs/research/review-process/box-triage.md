...
- **#262(すでに実測済み。導出をやり直さない)。`--allowedTools`に汎用`Read`/`Grep`/`Glob`が無く、
  ...
  **不一致は「設定値」ではなく「設定値から導いた帰結」にある。設定は宣言どおりでも、
  宣言された制約が実際には貫徹していない。**`.claude/skills/pr-review-flow/SKILL.md`117行は
  この制約を事実として扱い、「`common/`への複製・型の二重定義・成果物間の矛盾など差分の外を
  読まないと判定できない観点はCodeRabbit・Codex Cloud・Draft前セルフレビューが担う」という
  観点の担当割り当てをこの前提の上に組んでいる。**この前提が常に成り立つとは限らないことを
  今回の実測が示した。**

  **「意図的か/ずれか」は2つに分ける:**
  - **意図的なのは、汎用`Read`を渡していないこと**
  - **意図されていないのは、その制限が実際には貫徹していないこと。**C側は貫徹している前提で
    観点の担当を割り振っている
...
- **⑥ quota失敗時の文言と課金体系の食い違い。判定: 意図的ではない・ずれ、両方とも確定**
  ...
  **別立てで記録する追加の発見(用語の古さとは別の問題):**`docs/pr-review-flow-details.md`
  ...
  課金下だったことは裏付けられる。**現行課金(AI Credits)下では、この13という数値に対応する
  単位が存在しない**(使用モデルが非開示のため、AI Creditsでの1レビューあたりのコストは
  台帳からも導けない)。Draft先行という結論そのものは動かない(AI Creditsは有限=月$10分、
  CopilotはDraft中にレビューしないため、限定することの効果はどちらの課金体系でも残る)が、
  **その根拠に置かれている「13」という数値は現行課金下では意味を持たない。**「13」を現行の
...
- [x] 箱1の不一致それぞれに、「意図的か/ずれか」が記録されている(CodeRabbit指摘・2026-08-17、
      ...
      **不一致として列挙した項目と1対1で対応する最終形**)
      ...
      - **Claude#262=`.claude-pr/**`限定は意図的、その制限が「差分の外を読まない」という
        帰結として実際には貫徹していないことは未評価(2.1節。訂正あり)**
...
「設定は宣言どおりだが、宣言された制約(『差分の外は読まない』)が実際には貫徹していない」
という所見に置き換えた(2.1節・5節・6.3節を連動して修正)。
