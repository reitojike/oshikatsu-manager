...
- `[FC387]` = <https://github.com/theFactoryHQ/factory-careers/pull/387> — public、組織リポジトリ。
  観測日2026-08-15。quota超過メッセージのレビュー。check-runsは同commitに27件存在(Playwright等のCI
...
- `[EVITA1420]` = <https://github.com/FgForrest/evitaDB/pull/1420> — public、組織リポジトリ(evitaDB、
  OSSプロジェクト)。観測日2026-08-15。quota超過メッセージのレビュー。同commitのcheck-runsは6件
  (CodeQL等)存在。ruleset設定は非公開のため文脈不明。
...
| 2 | Copilot code reviewでカスタム指示を使うかどうかは個人設定でオン/オフでき、既定はオンとされる | [Adding repository custom instructions for GitHub Copilot](https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions) | A | 2026-08-15 | 確認済み |
...
| 5 | 一方、同一レビューのインラインコメントオブジェクトの `user.login` は `Copilot`(角括弧`[bot]`なし)であり、review単位と表記が異なる(`gh api repos/dancing-bear-show/dancing-bear/pulls/214/reviews/4939680421/comments` で確認) | `[DB214]` | C2 | 2026-08-15 | 確認済み |
| 5 | login表記が異なる2オブジェクトは同一の `id: 175728472` / `node_id: BOT_kgDOCnlnWA` を持つ。安定した識別子は `login` ではなく `id`/`node_id` である(review側 `user` と review-comment側 `user` を突合して確認) | `[DB214]` | C2 | 2026-08-15 | 確認済み |
| 5 | レビュー完了時、head commitにGitHub Actionsのcheck-run(`name: copilot-pull-request-reviewer`, `app.slug: github-actions`)が作成され、`status: completed` / `conclusion: success` を持つ(`gh api repos/chenrui333/homebrew-tap/commits/6b5bf1e95a8ca24028e2146181be95788ddb4c3d/check-runs` で確認) | `[HBT10215]` | C2 | 2026-08-15 | 確認済み |
...
| 6 | 「成功」「指摘0件」「quota超過でスキップ」を区別する専用の構造化フィールド(webhookイベント種別やAPIフィールド)は見当たらない。区別には `body` 本文の文言を読むしかない | `[SCOPE-RATELIMIT]` | A | 2026-08-15 | 公式に未文書化 |
| 6 | 正常完了時はcheck-run `copilot-pull-request-reviewer` が `success` で作られる一方、quota超過/サイズ上限超過で失敗した場合は同名のcheck-runが一切作られない(CIの盛んな他リポジトリでも0件)。`gh api repos/{owner}/{repo}/commits/{sha}/check-runs` で各head shaのcheck-runs総数・名称を確認した | `[LFSX66]`(15件のcheck-runあり、Copilot分0件)、`[FC387]`(27件のcheck-runあり、Copilot分0件) | C2 | 2026-08-15 | 確認済み |
...
| 6 | Copilotが再レビュー(push後の2回目以降のレビュー)を行う際、`pull_request_review` webhookが新規レビューとして `submitted` で届くか、既存レビューの更新として `edited` で届くかは未調査(本調査では複数回Copilotレビューが走った実例を観測できなかった。軸1参照) | 未調査。追試手段: `review_on_push` を有効化したリポジトリでpushを2回行い、webhookまたは `/pulls/{n}/reviews` のreview件数増分を確認する | D | 2026-08-15 | 未調査 |
...
| 7 | ユーザーのpremium request/AI Creditsのquotaが尽きた場合、レビューは黙って再試行・キューイングされるのではなく、専用の拒否メッセージ付きレビューが即時投稿される(逐語文字列は出力パターン表を参照) | `[QUOTA-LIST]` | C2 | 2026-08-15 | 確認済み |
...
| 9 | 「低確信度のため抑制されたコメント」は、review本文中の折りたたみブロック `<details><summary>Suppressed comments (N)</summary>` の中にのみ存在する。`/pulls/{n}/comments` や `/reviews/{id}/comments` などコメント一覧系エンドポイントには一切現れない(review本文は「generated 2 comments」+「Suppressed comments (1)」と記載するが、`gh api repos/dancing-bear-show/dancing-bear/pulls/214/reviews/4939680421/comments` は厳密に2件のみを返し、抑制分の1件は含まれない) | `[DB214]` | C2 | 2026-08-15 | 確認済み |
| 9 | ファイル別サマリ表も同様にreview本文中の折りたたみブロック `<details><summary>Show a summary per file</summary>` にのみ存在し、別個の構造化フィールドではない(同PRのreview本文をhtml_url `https://github.com/dancing-bear-show/dancing-bear/pull/214` で目視確認) | `[DB214]` | C2 | 2026-08-15 | 確認済み |
...
| 9 | インラインコメントの `line` が `null` になるケースがあり、その場合は `diff_hunk` と `position`/`original_position`(diff内の相対位置、pushのたびにずれうる)が実質的な位置情報になる(`gh api repos/deephaven/deephaven-core/pulls/8367/comments` の1件目で確認) | `[DHC8367]` | C2 | 2026-08-15 | 確認済み |
| 10 | 「レビューが1回走った」の一次的な痕跡は `/pulls/{n}/reviews` に追加される1件のreviewエントリ(`user.login: copilot-pull-request-reviewer[bot]`、`submitted_at` 付き) | `[DB214]`、`[HBT10215]`、`[DHC8367]`、`[MAS5671]` の各PRで `gh api repos/{owner}/{repo}/pulls/{n}/reviews` を確認 | C2 | 2026-08-15 | 確認済み |
...
| 10 | 副次的な痕跡としてGitHub Actionsのcheck-run `copilot-pull-request-reviewer` があり、`started_at`/`completed_at`/`status`/`conclusion` とActions run/jobへのリンクを持つ。review オブジェクト自体には所要時間や成否のconclusionが無いため、これが唯一の実行時間ソースになる(started_at 2026-08-14T17:20:31Z, completed_at 2026-08-14T17:21:40Z, 所要約69秒) | `[HBT10215]`(`gh api repos/chenrui333/homebrew-tap/commits/6b5bf1e95a8ca24028e2146181be95788ddb4c3d/check-runs` で確認) | C2 | 2026-08-15 | 確認済み |
...
| 11 | 個人(組織非所属)ユーザーが「現在の期間のAI Credits残量/premium request残量」を機械的に読める公開APIエンドポイントは見当たらない。billing UI(`github.com/settings/billing`)の存在は確認できたが、対応するREST/GraphQLエンドポイントの案内は見つからなかった | `[SCOPE-BILLING]` | A | 2026-08-15 | 公式に未文書化 |
...
| check-run(commits/{sha}/check-runs) | (存在しないこと自体が痕跡) | (該当なし。`copilot-pull-request-reviewer` という名のcheck-runが1件も無い) | quota超過/サイズ上限超過でレビューが失敗した場合、CIの盛んな他リポジトリ(check-run 15〜27件)でもこの名前のcheck-runは一切作られない | `[LFSX66]`(15件のcheck-runあり、Copilot分は0件)、`[FC387]`(27件のcheck-runあり、Copilot分は0件) | C2 | 2026-08-15 | 確認済み |
