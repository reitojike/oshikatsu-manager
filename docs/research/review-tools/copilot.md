...
- `[EVITA1420]` = <https://github.com/FgForrest/evitaDB/pull/1420> — public、組織リポジトリ(evitaDB、
  ...
  2026-08-15の再取得時点で7件(CodeQL等)存在(**件数は観測時点の値であり増減する**)。
  ruleset設定は非公開のため文脈不明。
...
user who requested the review has reached their quota limit.` を完全一致で確認した**15件のPR。ただし
リポジトリ単位では14で、独立ではない** —— `FerrLabs/LFSX` が PR 66 と PR 65 の2件を占める
(いずれもpublic、observed 2026-08-14の17時台UTC、各リポジトリのruleset設定は非公開
...
| 5 | login表記が異なる2オブジェクトは同一の `id: 175728472` / `node_id: BOT_kgDOCnlnWA` を持つ。**観測した範囲で一致していたのは `login` ではなく `id`/`node_id` だった。ただしこの一致単独からは「安定した識別子である」という一般仕様までは導けない** —— `id`が`login`より耐久性のある識別子であること自体はGitHub公式ドキュメントに明記されている(["Get a user using their ID"](https://docs.github.com/en/rest/users/users?apiVersion=2022-11-28): "This method takes their durable user ID instead of their login, which can change over time."2026-08-16に`curl`で本文を取得し確認。**当初この一般仕様の根拠を出典無しで書いていたため、CodeRabbitの指摘で気づき追加した**)が、単一時点・単一PRでの一致観測それ自体はこの一般仕様を証明する材料ではない(review側 `user` と review-comment側 `user` を突合して確認) | `[DB214]` | C2 | 2026-08-15 | 確認済み |
...
| 6 | **「成功」「指摘0件」「quota超過でスキップ」を区別する専用の成功/失敗フィールドは確認できなかった。**専用の構造化フィールド(webhookイベント種別やAPIフィールド)への言及は`[SCOPE-RATELIMIT]`(トラブルシュート・usage limitsの解説ページ)には無い。**この2ページは散文の解説ページであり、GitHubのwebhook/APIスキーマを網羅的に確認したものではないため、フィールドの不在そのものを保証しない**(文書化されていないにとどまる)。**判定に使える信号は`body`本文の文言(直接的)に加え、check-run(`copilot-pull-request-reviewer`)の不在が補助信号として使える。`state`は補助信号にならない**(**当初「判定手段は`body`本文の文言のみ」と断定していたが、同じ台帳の軸6の別行(check-run不在の観測、直下の行)を無視した言い過ぎだった。CodeRabbitの指摘で気づき、check-run不在を補助信号として書き加えた。ただしそのとき`state`の一致・不一致も補助信号であるかのように書いてしまったが、直上の行が示すとおり`state`はquota超過時も成功時と同じ`COMMENTED`の一定値であり、一致・不一致という比較対象自体が存在しない。claude-reviewの指摘で気づき訂正した。check-run不在はquota超過の2件でのみ観測されており、行数上限超過等の他の失敗理由でも同様かは未確認**) | `[SCOPE-RATELIMIT]`。検索語: "code review", "skip", "status"。両ページとも散文の解説ページ(開いた情報源)であり、不在は「文書化されていない」にとどまる。C2の裏付けは`[QUOTA-LIST]`・`[DB214]`・`[HBT10215]`・`[LFSX66]`・`[FC387]`(check-run不在の観測は直下の行) | A | 2026-08-15 | 公式に未文書化 |
...
| 6 | 再レビューが `pull_request_review` webhook 上で `submitted` として届くか `edited` として届くかは未調査。**ただし上記のとおりレビューオブジェクト自体は追加されるため、`/pulls/{n}/reviews` の件数増分では再レビューを検知できる** | 未調査。追試手段: `review_on_push` を有効化したリポジトリでpushを2回行い webhook ペイロードを確認する | D | 2026-08-15 | 未調査 |
...
| 8 | Copilot code reviewは「GitHub Code Quality」(CodeQLによるルールベースの静的解析)とは別製品であり、個別に有効化する(**当初ここまでに加えて「Copilot code reviewの出力にCodeQLのルールベース検出結果は含まれない」と断定していたが、軸3で採用した表現(現行資料からは実施を確認できない、にとどめる)と強さが揃っていなかった。CodeRabbitの指摘で気づき、この行も同じ強さに揃えた**)。**現行の「About GitHub Copilot code review」ページは「GitHub Code Quality complements Copilot code review by adding: Rules-based CodeQL-powered analysis on pull requests and on your default branch.」と述べており、少なくとも別製品として追加される形であることは確認できる。ただし2025-10-28のchangelogはCCR自身へのCodeQL/ESLint統合を予告していたため(軸3参照)、現行の一次情報からは「CCRの出力自体にCodeQLの検出結果が含まれるか」を確認できない、にとどめる(軸3の推論と根拠は同一。「含まれない」と断定するものではない)** | [GitHub Code Quality](https://docs.github.com/en/code-security/concepts/code-quality/code-quality)、[About GitHub Copilot code review](https://docs.github.com/en/copilot/concepts/agents/code-review)(2026-08-16、`curl`で本文を取得し確認) | A | 2026-08-16 | 確認済み |
...
| check-run(commits/{sha}/check-runs) | (存在しないこと自体が痕跡) | (該当なし。`copilot-pull-request-reviewer` という名のcheck-runが1件も無い) | **quota超過で**レビューが失敗した場合、CIの盛んな他リポジトリ(check-run 15〜28件)でもこの名前のcheck-runは一切作られない(**サイズ上限超過のcheck-run不在は観測していない。この行の観測はquota超過の2件のみである**) | `[LFSX66]`(15件のcheck-runあり、Copilot分は0件)、`[FC387]`(28件のcheck-runあり(観測時点の値。件数は増減する)、Copilot分は0件) | C2 | 2026-08-15 | 確認済み |
