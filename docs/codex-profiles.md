...
**`mcp__codex__codex`(MCP)経由では階層を指定できない。**codex-cli 0.147.0 で
考えられる経路をすべて実測した(issue #163)。
...
**何も返らない場合は分類できないので、同じ内容を1回だけ `codex exec -p <階層名>` に渡し、
それでも失敗したら通常の3回ルール(`AGENTS.md`)に従う。**
