...
| タスク | 状態 |
| --- | --- |
| **Claudeレビューをrequired status checkへ配線** | **完了(2026-08-12)。** issue #95 で無投稿検知を実装し、Rulesetの「Require status checks to pass」に `Claude Review / claude-review` と `PR Template Check / check` を追加した(PO操作)。**赤はマージをブロックする。**「Require branches to be up to date before merging」はOFFのまま、`codex-review` は追加しない(workflowごと削除済み)。赤くなる条件・型の切り分けは`docs/pr-review-flow-details.md`「Claude Review」を参照 |
