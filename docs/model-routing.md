...
**RLS・権限マトリクスに触れる作業を渡すときは、`docs/permissions.md`「権限に関わる
コードを触る前に必ず読む」を必読資料として引き継ぎに含める。**Solへの整合性監査だけでなく、
**RLSを実装するTerraへの引き継ぎにはより強く効く**(コードに実際に触れるのはTerra側のため)。
`CLAUDE.md` では複数箇所で繰り返し念押しされているが、この文書はそれを継承していないため、
この文書だけを読んで引き継ぎ計画を書くと書き漏らしうる。
...
5. **ラベルとProjectの `Model` を切り替え先に書き換える。**担当が変わったので、
   `CLAUDE.md`「Issue全体の担当が変わったとき」の扱いに従う。**書き換える対象は、
   Codex側のラベルが実際に付いていた層。**Sub-issueに分担して渡していた作業がフェイルオーバーで
   止まった場合はそのSub-issue自身を書き換え、親Issueには触れない
