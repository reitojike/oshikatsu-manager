...
> **階層を選んでCodexに渡す呼び出しは、CLI(`codex exec -p <階層名>`)で行う。**
> `agent:sol` / `agent:terra` / `agent:luna` の作業を `mcp__codex__codex`(MCP)で投げない。
> **MCPにはプロファイル名で階層を指定する経路が無く、指定しなければ既定で最上位階層になる**
> (実測。issue #163)。
> **指定漏れの挙動、MCPで試した経路と結果、採らない理由、確認手順は
> `docs/codex-profiles.md`「呼び出し方」「階層を指定できるのはCLIだけ」を参照する**
> (ここには書き写さない)。
