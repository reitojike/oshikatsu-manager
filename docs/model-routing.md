...
- **記録先**: **月次Issueを1本起票し、そのコメントに記録する。**タイトルは
  `消費実績の振り返り(YYYY-MM)`、`phase:0` と担当の `agent:*` を付ける。
...
1. **`agent:*` ラベルの内訳。**`closed`(実際に回した分)と `open`(これから回す分)を分けて数え、
   Codex側(`sol` / `terra` / `luna`)とClaude側(`opus` / `sonnet` / `haiku`)の合計も出す
2. **上限到達とフェイルオーバーの記録。**どちらのプロバイダが何回止まったか、リセットまでの実測時間、
   その間に横移動したIssue番号(記録の作法は
   `docs/model-routing-details.md`「上限到達時に読む手順」の手順4・5)
3. **action参照のcommit SHAを更新すべきかの確認。**Dependabot / Renovateは導入せず、
   **この月次振り返りに相乗りして手動更新する**(PO判断・2026-08-11)。
...
**件数比のずれ単体はトリガにしない。**比はタスクの性質(判断中心の月かどうか)で動き、
母数も小さい。**比を見て動くのは、上のトリガが引かれた後の解釈材料としてだけである。**

**なぜ月次なのか、必須3項目に絞った理由、記録した数字を読み違えないための制約(件数比と枠の
消費比は別物であること等)、集計コマンドは `docs/model-routing-details.md`「配分を振り返るときに読む手順」。
ここには書き写さない。**
