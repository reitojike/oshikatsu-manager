...
- **実施者**: 集計は **Luna / Haiku相当**(`gh` の出力だけで正誤が決まる)。
...
**必須の3つは、すべて `gh` とIssueコメントだけで取れる。**機械集計を前提にした項目を必須にしない。
...
3. **action参照が**commit SHAで固定されているかの確認**と、固定済み参照の更新要否の確認。**
   **未固定の参照が残っていたら、それ自体が `AGENTS.md`「automation-config」のP0であり、
   別Issueを起こす。**「更新対象が0件」と「固定されていないので更新対象が存在しない」は別物である。
   **固定済みの判定は「`@` の後ろが40桁のcommit SHA」で行う**(`uses:` の行を数えるだけでは
   タグ・ブランチと区別が付かない)。`uses: ./...` のローカルactionは対象外。
