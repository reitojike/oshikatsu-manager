...
`docs/model-routing.md` / `docs/task-management.md` / `AGENTS.md`)の記述が矛盾している、
またはどちらとも取れる判断が
必要になったときの進め方。
**判断を含む実装に着手する前に読む。**
...
このリポジトリの品質保証は「壊れたらCIが赤くなる」に依存している(`AGENTS.md`)。
...
4. **MVPに入れる / 拡大期に送る、の線引きが動く。**スコープは工数ではなく製品の判断
...
- 判断が外れてもコードを消せば元に戻る範囲に収まる
  ...
  **「コードを消せば戻る」は、残るものがコードだけの場合に限る。**
  ...
  (`AGENTS.md`「参加登録の公開設定はデフォルト非公開。既定値を反転させても何もエラーにならない」)
...
- Opusに回るタスクは定義上「何を選ぶか」を含む(`AGENTS.md`)。
...
作業のはずである(`AGENTS.md`)。成果物が割れていることに気づいたら自分で選ばず、
`AGENTS.md` のエスカレーション手順でOpusに上げる。上げる際は、どの成果物のどこが
割れていたかを引き継ぐこと。
...
その層がこの節の一般則どおりOpusまで上げる。理屈は
`docs/task-management.md`「Issue内でモデル階層を分担する」に1か所だけ置く
(重複した記述は片方だけ古くなる)。
