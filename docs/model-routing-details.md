...
2. 下記「エラー原文の記録範囲」に従って、実際に返った内容をIssueに記録する
3. Projectの `Status` を `Blocked` にする(上限到達と異なり、これ以上自律的に進められない)
4. 人間の判断を待つ。応答がないことを理由に自分の判断で「上限到達」へ格上げしない

### エラー原文の記録範囲

**Issue・引き継ぎprompt(いずれも本リポジトリはpublic)には、エラー応答の原文をそのまま
貼らない。**次の4項目だけを記録し、これ以外は記録しない(マスクして残すのではなく、
そもそも書き写さない)。

- 判定に一致した判別文字列(上表の「判別」列に一致した部分だけ。前後の文脈は含めない)
- リセット時刻の表記(読み取れた場合。読み取れなければ「不明」と明記する)
- 判定した分類名(上表の「分類」列のいずれか)
- 失敗した階層と、渡した内容(呼び出し側が把握している情報であり、応答本文からの引用ではない)

**この4項目の外側を一切記録しないこと自体が、マスクの正しさを機械が判定できない問題を消す。**
「秘密を検出して隠す」(ブロックリスト)方式は、列挙していない形式の秘密(将来追加される
認証方式、想定外のURL構造)が漏れる余地を原理的に残す。「安全と分かっている項目だけを
書き写す」(アローリスト)方式にすれば、その余地自体が無くなる。

**上記「最初にある分類に実際に遭遇したら、判別に使った実際の文言をこの表に追記して更新する」
運用は、この4項目のうち「判定に一致した判別文字列」の範囲に収まる。**表の更新に応答本文
全体は要らない。
