...
> **実測で確認した**(issue #163、codex-cli 0.147.0)。実モデルslugを直接渡せば技術的には
> 選べるが、ルーター層になるため運用としては採らない。試した経路と結果、MCP経由を
> 使ってよい範囲、無反応時の扱いは `docs/codex-profiles.md`「階層を指定できるのはCLIだけ」。
> ここには書き写さない。
