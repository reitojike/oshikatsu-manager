...
2. 下記「エラー原文の記録範囲」に従って記録する。**判別文字列は定義上存在しない**
...
運用は、既に文字どおりのトークンを持つ行(上限到達、一時的なレート制限の`` `rate limit` ``/
`` `retry-after` `` 部分)にはそのまま当てはまる。表の更新に応答本文全体は要らない。**
...
**文字列判定だけで断定しない。**上限到達の実測文言(上表)かリセット時刻のいずれも読み取れない
場合は、**上限として断定せず「不明」として扱う**(上記『「不明」に分類したときの扱い』)。
