...
| 上限到達 | 本文に `You've hit your usage limit`(実測)**と** `try again at <時刻>`(実測)の**両方**が揃った場合。`limit reached` / `quota` は未確認 | 同階層のClaudeへ即フェイルオーバー | **`codex exec`(CLI)・`mcp__codex__codex`(MCP)の両経由で実測**(2026-08-11、#153・#86)。文言は両経路で一致した |
...
- **判定に一致した判別文字列。**ただし上表の「判別」列が`` `rate limit` ``・
  ...
  **`` `try again at <時刻>` `` はプレースホルダーを含むため全体としては対象外だが、
  `<時刻>`を除いた固定の前置き部分(`try again at`)は文字どおりのトークンとして
  転記可とする**(詳細は下記「上限到達行に書かれた…」を参照)。
...
- **リセット時刻の表記。**ただし次のいずれかの形式に完全一致する部分だけを記録する。
  ...
  日時表記(例: `Aug 16th, 2026 8:32 AM`。UTC/ローカルいずれかは下記「未確認」のまま扱う)、
...
文字どおり一致することはない。**上表のとおり、この行への一致は
`` `You've hit your usage limit` `` **と** `` `try again at <時刻>` `` の**両方**が
応答本文に現れた場合に限る(PO確認2026-08-11、Issue #126)。片方だけしか読み取れない場合は
上限到達に一致せず、「不明」として扱う(上記『「不明」に分類したときの扱い』)。
...
4. **Issueにコメントする。**書くのは、**到達日時**、どの階層がどのエラーで止まったか、
   リセット時刻が読み取れたならその時刻(上記「エラー原文の記録範囲」の許容形式に完全一致する
   場合のみ。一致しなければ「不明」と明記する)、
...
- 上記「エラー原文の記録範囲」に従って記録した内容(判定に一致した判別文字列・判定した分類名)。
  応答本文の原文はそのまま含めない
- リセット時刻が読み取れていればその時刻(上記「エラー原文の記録範囲」の許容形式に完全一致する
  場合のみ。一致しなければ「不明」と明記する)
