...
| 不明 | 上のどれでもない | **連続リトライしない。**下記「エラー原文の記録範囲」に従って記録し、人間に返す | 対象外(このリストに一致しない場合の受け皿) |
...
データ(いつ再試行してよいかを示す時刻・待機時間)であり、上記の許容形式(数値の待機秒数、
または日時表記)に完全一致する場合に限り、その一項目として記録してよい。
判別文字列としての転記可否と、リセット時刻としての記録可否は別の軸だが、後者にも
形式の制約が掛かる点は上記「リセット時刻の表記」のとおり。
...
**上限到達行の`` `try again at <時刻>` ``は、`<時刻>`部分がプレースホルダーであり、
文字どおり一致することはない。**この行での「一致」は、`<時刻>`を除いた固定の前置き部分(`` `try again at` ``の後に
時刻が続く形)が応答本文に現れることを指す。**`<時刻>`に対応する実際の時刻表記は
判別文字列ではなく、「リセット時刻の表記」(上記「エラー原文の記録範囲」)として別に扱う。**
つまりこの行は、固定の前置き文字列(判別文字列として転記可)と、可変のリセット時刻
(リセット時刻として転記可)の2つに分解して記録する。
