...
2. 下記「エラー原文の記録範囲」に従って、対象Issueにコメントする。**判別文字列は定義上存在しない**
   ...
   リセット時刻の有無・渡した内容の要約に留まる。**原文を人間が直接確認する必要があるときは
...
問題を消す。**「秘密を検出して隠す」(ブロックリスト)方式は、列挙していない形式の秘密
(将来追加される認証方式、想定外のURL構造)が漏れる余地を原理的に残す。「安全と分かっている
項目だけを書き写す」(アローリスト)方式にすれば、その余地自体が無くなる。
...
固定トークンにならない)ため、この転記可の対象に含めない。**一時的なレート制限に遭遇したときも、
実際に返ってきたretry-after値・時間表記は転記せず、分類名「一時的なレート制限」だけを記録する。**
...
アローリスト方式がブロックリスト方式を退けた理由(検出漏れが機械で判定できない)と同型のリスクを、
