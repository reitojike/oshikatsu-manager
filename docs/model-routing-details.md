...
にはそのまま当てはまる。表の更新に応答本文全体は要らない。**`retry-after`は上表(108行目付近)で
...
**意味の説明の行(認証・権限/サンドボックス拒否/コンテキスト超過)を初めて確定させるときは、
上記の転記制限の対象外にはならない。**Issue・引き継ぎpromptへの転記は上記のとおり禁止したままで、
**表そのものの更新は別の経路で行う。**この文書(`docs/model-routing-details.md`)自体への
編集は通常のPRを経るため、応答本文を直接読める人間(PO、またはこの判断を担うOpus階層)が、
秘密・非公開パスを含まない最小限の部分文字列だけを判別トークンとして切り出し、
その部分文字列だけをコミットする。**切り出す前の応答本文全体は、Issueにもpromptにも、
このPRのコミット・コメントにも書かない。**この手順を経て初めて、その行は上記の
「文字どおりのトークン」に昇格し、以後の記録・転記で扱える対象になる。
