...
(CLI)経由で実際に返った文言を確認した。**それ以外の行(認証・権限/サンドボックス拒否/
コンテキスト超過/一時的なレート制限/不明)は依然として未確認で、判別文字列は推測に基づく
暫定のままである。行ごとの実測状況は表の最終列で読み分ける。
...
| 上限到達 | 本文に `You've hit your usage limit`(実測)、`try again at <時刻>`(実測)。`limit reached` / `quota` は未確認 | 同階層のClaudeへ即フェイルオーバー | **`codex exec`(CLI)経由で実測**(2026-08-11、#153)。`mcp__codex__codex`(MCP)経由は未確認 |
| 不明 | 上のどれでもない | **連続リトライしない。**原文をIssueに貼って上位へ返す | 対象外(このリストに一致しない場合の受け皿) |

**上限到達行の実測文言は `codex exec`(CLI)経由でのみ確認したものである。**
`mcp__codex__codex`(MCP)経由で実際に返る文言はまだ確認されていない。経路が違えば
ラッパーの整形が異なりうるため、両者を同一視しない。
