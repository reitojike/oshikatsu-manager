...
**Codex Cloudについて。**PR #169・#170・#173・#174(2026-08-11)の実測では、Ready化そのもの
(pushを伴わない`gh pr ready`単体)でも**約3分後に自動投稿される**ことを4件で再現した
(PR #113での「8分以上経っても自動投稿が無かった」という初出の実測は、それより短い待ち時間で
打ち切っていた可能性が高く、本節はこの実測で置き換える)。**したがって`gh pr ready`単体でも
自動発火のトリガーになるため、直前の打ち切り判定巡(上記「Draftフェーズ」)で得たCodexの結果を
そのまま充当してはならない。**Ready化した時点のHEADに対して、Ready化自体が新しい自動投稿を
発火させる。**`gh pr ready`の後は最低5分待ってから、そのHEADのSHAに対するCodexの投稿を
確認すること。**5分待っても該当コメントが無い場合に限り、`＠codex review`を手動コメントする。
