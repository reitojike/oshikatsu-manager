...
**マージ前にCodexの結果を最低1回得ることを義務とする**(Ready化後のpushによる自動投稿、
Ready化後の手動コメント、または上記「Draftフェーズ」の**打ち切り判定巡で得た結果**の
いずれかで満たせばよい。打ち切り判定巡の後にpushが無いままReady化する典型パスでは、
その結果をそのまま充当してよく、Ready化時に改めて呼ぶ必要はない。打ち切り判定巡の後に
コード変更を含むpushをした場合は、そのpushで改めてこの義務を満たす)。**「得た」とみなすのは、
Codexがコメントを投稿した(指摘あり)、または👍リアクションを付けた(指摘0件)場合に限る。**
どちらも無い状態は未実行として扱い、指摘0件と読み替えない(Codex自身の案内文言
「suggestionsがあればコメント、無ければ👍」に基づく。上記「Draft前セルフレビュー」の
