...
3. **`/code-review`を使うのは、`common/`・権限/RLS・`supabase/migrations/**`のいずれかに
   ...
   **走らせ方は経路によらず統一する。Agentツールで新規サブエージェントを起こし、下記
   「レビュアーへ渡す最低限のコンテキスト」を渡して走らせる。実装に使った対話セッション内で
   直接`/code-review`を呼ばない(上記2と同じ理由。issue #102)。**新規サブエージェントは
   同じモデル・同じリポジトリ文書を読むため、別プロバイダのCodexほどの独立性は無い
   (#102が塞ぐのは会話上の前提の持ち越しであって、モデルの多様性ではない)。
   levelは`medium`を既定とし、3分類経路に該当するPR、または「Draftフェーズ」節が定める
...
- PR種別(issue #95の3分類。当てる観点がこれで決まる。新規セッション・新規サブエージェントは
  GitHub Issueスレッドを読めないため、渡す側が対象パスから該当する分類名を伝える):
...
- GitHub上のコンテキスト(PRコメント・CI結果・レビュー履歴)がまだ存在しないことの明示
  (レビュアーが「見えないだけ」なのか「無い」のかを区別できないため)
...
4. 呼び出し失敗・コンテキスト超過・空回答・**Codexの利用上限到達**は「レビュー済み」に
   数えない。上記3の対象PR(`common/`・権限/RLS・`supabase/migrations/**`、または
   上限到達したPR全般)では`/code-review`(新規サブエージェント)に切り替え、それ以外の
   PRでは新規のローカルCodexを再実行する。**切り替え先(`/code-review`)でも同じ失敗が
   起きた場合は、この4の条件をそのまま適用して再実行する。ただし切り替え先で起きた失敗が
   Claude側の利用上限到達だった場合は再実行せず、`docs/model-routing-details.md`の既定
   (Blockedにして待つ)に従う。**同じ問題が3回続く場合は`AGENTS.md`のエスカレーション基準
   (Sonnet→Opus)に従う
