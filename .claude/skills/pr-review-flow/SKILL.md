...
**「得た」とみなすのは、コメント本文の`Reviewed commit`にHEADのSHA(短縮形でよい)が
一致するコメントをCodexが投稿している場合に限る。**PR #168〜#171(Draft中5巡すべて)の実測では、
...
**将来、👍のみの投稿が実際に観測された場合は、そのコメントがどのコミットに対する反応かを
確認できる手段(GitHub APIのリアクション対象、または投稿タイミングと直前pushの前後関係)が
無い限り「得た」と扱わないこと。**
