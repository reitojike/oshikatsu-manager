...
5分待っても(下記の「得た」の条件を満たす)該当コメントが無い場合に限り、
...
これを防ぐため、マージ実行時は4面を取得した時点のHEAD OID(`gh pr view {number}
--json headRefOid --jq .headRefOid`)を`gh pr merge {number} --match-head-commit <OID>`
に渡す。取得後に新しいpushがあった場合はマージ自体が失敗するため、古い確認結果のまま
気づかずマージすることを防げる。
