...
投稿する)が、その時点のHEADのフルSHAの前方一致(prefix)であるコメントをCodexが
投稿している場合に限る。**短縮形どうしの一致だけでは同じ接頭辞を持つ別コミットを
誤って一致と判定しうるため、必ずHEADのフルSHAを取得したうえで前方一致を確認する。
PR #168〜#171(Draft中5巡すべて)の実測では、
