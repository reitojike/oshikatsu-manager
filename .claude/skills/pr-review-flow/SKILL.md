...
(定型文+`commit_id`)と面4のインラインコメント(指摘の詳細)の両方が投稿された。Ready化
から約3分後で、上記の5分待機の範囲内)の2件が投稿されていた。「`Reviewed commit`が一致する
...
指摘0件のときは面2(issue comment、例: PR #173の15:38:54Z)、指摘ありのときは面3(定型文
のみが入り、`commit_id`フィールドで直接フルSHAが得られる)**と同時刻に**面4のインライン
コメント(指摘の詳細)が投稿される実例がある(例: PR #173の15:46:51Z。上記「Ready化」節
参照)。上記「得た」の判定で`Reviewed commit`を探す際は、面2・面3の両方を対象に含める。
