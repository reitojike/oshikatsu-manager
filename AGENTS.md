...
- **Claude / Codex どちらのプロバイダに振るか → `docs/model-routing.md`。
  タスクの担当を決める前に必ず読む**
- **併用の根拠と、上限到達時のフェイルオーバー手順、月次の配分振り返りの手順
...
- `.github/scripts/` — CI・レビューボットの判定ロジック。`common/`/`lib/`/`app/`/`mcp/` の
  依存の向きのルールは掛からないが、pure関数切り出しとテストの対象範囲(`docs/testing.md`)には入る。
...
リポジトリの方針に沿っているかを確認し、変更ファイルに該当する分類の観点をすべて併用する。
以下は、このリポジトリでP0/P1として指摘すべき問題の定義である。
同一根拠の指摘は差分全体へ横展開し、複数箇所を列挙して1件に集約する。
コードを自動修正しない。セルフレビューで処置済みの項目は再掲しないが、処置への反論はよい。

### code

次はP0/P1として扱う。

- `as`、`any`、抑制コメントで型・lintエラーを黙殺している
- `common/`に置くべき判断ロジックを別の層に置く、または複製している
- 静かに失敗する変更にテスト、特に否定側のテストが不足している
- DB生成型またはZod由来型を使わず、型を二重定義している

### governance-docs

ガバナンス文書の不備は後続作業を誤った方針へ導いても機械的に止まらないため、次はP0/P1として扱う。

- 成果物間に矛盾がある
- `docs/decision-policy.md`の確認対象を、確認せず決めている
- 同一ルールの正本を複数箇所に置いている

索引、短い要約、正本への参照は重複と扱わない。

**上の「矛盾」と「正本の複数配置」は、読んだ実行者が取る行動が変わるものだけを指す。**
行動が変わるならP0/P1として扱う。同じ結論に至るのに言い回し・表記・レンダリングだけが違うものは
**軽微**とし、P0/P1に上げない。判定はこの一線だけで行う ——
**読んだ人/エージェントが、直す前と直した後で違う行動を取るか。**
軽微と判定した指摘のDraftでの扱い(打ち切りと「見送り(軽微)」としての記録)は
`pr-review-flow` skillを参照(governance-docs分類のPRに限る)。
...
- 権限を拡大している
- secret未設定やworkflow検証のskipがsuccessに見える穴がある
- イベント種別や再実行条件が意図と一致しない
- required checkが永久にpendingになり得る
- fork PRでのsecretの挙動を考慮していない
- action参照をcommit SHAで固定していない
- 設定変更と関連文書・テストが整合していない
...
**PRはまずDraftで作成する。**Draftでレビューボットと反復し、反復の終了条件を満たしてから
...
- **1つのIssueの中でも、判断するフェーズと、決まったとおりに反復するフェーズは分離できる。
  ...
  **残り作業の合否を判定する根拠が、引き継ぎ先の層で完結しているか。**
  ...
  穴埋めテンプレートは `docs/handoff-template.md`。項目の定義は
  `docs/task-management.md`「引き継ぎ計画に必ず書くこと」が正本で、テンプレート側には転記しない。
